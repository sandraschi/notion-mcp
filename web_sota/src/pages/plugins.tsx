...
import { CheckCircle2, Cpu, Plus, Zap } from "lucide-react";
...
export function Plugins() {
  ...
  const fetchPlugins = async () => {
    try {
      ...
      console.warn("Plugins fetch failed", error);
    }
  };

  const installPlugin = async (id: string) => {
    try {
      await fetch(API_BASE + `/api/plugins/install/${id}`, { method: "POST" });
      fetchPlugins();
    } catch (error) {
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-slate-400">
          Extend NotionMCP with SOTA integrations and local scripts
        </p>
      </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {installed.length === 0 ? (
            <div className="col-span-2 p-8 border border-dashed border-slate-800 rounded-lg text-center text-slate-500">
              No local plugins active. Start by building a script in the
              `plugins/` directory.
            </div>
          ) : (
      ...
      <div className="space-y-4">
        <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-widest">
          SOTA Marketplace (Notion Ecosystem)
        </h3>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {available.map((plugin) => (
            <Card
              key={plugin.id}
              className="border-slate-800 bg-slate-950/50 hover:bg-slate-900/50 transition-all group"
            >
